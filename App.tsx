...
const App: React.FC = () => {
  ...
  const [answers, setAnswers] = useState<Answers>({ currency: 'USD' });
  ...
  const handleAnswerChange = useCallback((id: string, value: string | number) => {
  ...
  const calculateResult = async () => {
    ...
    setError(null);
    try {
        const stabilityMultipliers: { [key: string]: number } = {
            'Very Stable': 3,
            'Stable': 6,
            'Unstable': 9,
        };
        const expenses = Number(answers.expenses) || 0;
        const stability = answers.jobStability as string;
